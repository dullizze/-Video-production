...
- [ ] **B4 — 사용자/쿼터 최소 구조**. 인증/결제 전 단계로 `user_id`, plan, monthly quota, usage count를 manifest/API 레벨에 얇게 추가해 SaaS 과금 구조의 자리 만들기.
...
- **API/대시보드(B1~B3)**: `docker compose up api` 후 `http://localhost:8000/`에서 대시보드, `/docs`에서 API 확인. `POST /jobs`는 기본적으로 background 실행을 예약하고, `auto_start=false`로 manifest만 생성 가능. `POST /jobs/{job_id}/run`은 기존 job 실행/재실행. 아직 Redis/Celery 없음, API 프로세스가 내려가면 실행 중 작업도 중단될 수 있음.
