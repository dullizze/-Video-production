...
- [ ] **B3 — 최소 웹 대시보드**. 로그인/결제 없이 주제 입력, 템플릿 선택, 생성 요청, job 상태 폴링, mp4 미리보기까지 가능한 내부용 첫 화면.
...
- **API(B1/B2)**: `docker compose up api` 후 `http://localhost:8000/docs`에서 확인. `POST /jobs`는 기본적으로 background 실행을 예약하고, `auto_start=false`로 manifest만 생성 가능. `POST /jobs/{job_id}/run`은 기존 job 실행/재실행. 아직 Redis/Celery 없음, API 프로세스가 내려가면 실행 중 작업도 중단될 수 있음.
