...
- 파이프라인 흐름: ① 주제 → ② 대본(Claude) → ③ TTS(edge-tts) → ④ 이미지(Pollinations) → ⑤ 자막(faster-whisper) → ⑥ 렌더(Remotion).

## 3. 다음 작업 (우선순위, PRD §14)
- [ ] **A1 — 자막 구문 그룹핑 + faster-whisper 제거** (PRD §7). 타이밍을 TTS 단어경계로 바꾸고, 어절을 "한 호흡=한 줄"로 묶는다. **완료기준: 어절 단위 단발 자막 0건, 한 줄 ≤16자/≤4어절/≤2.5초.**
- [ ] **A3 — 유료 이미지(Flux)·유료 TTS 연동 + 폴백.** 완료기준: 안정적 5장 < 30초, 보이스 교체 가능.
...
## 5. 함정 / 실무 메모 (꼭 읽을 것)
- **Pollinations.ai**: 익명 티어가 동시 요청을 `402 Payment Required`(rate limit)로 막음 → `pipeline/steps/visuals.py`는 **순차+1초 간격**으로 처리. 신규 5장 다운로드가 느림(최대 ~6분). → v2에서 **유료 Flux로 대체** 예정.
...
- **자막 문제(미해결, A1 대상)**: 현재 faster-whisper(`small`, `.env` `WHISPER_MODEL`)가 **어절마다** 자막을 냄. 쇼츠답지 않음 → A1에서 구문 그룹핑으로 교체.
