...
- **B1 완료**: FastAPI 파일 기반 job API 스켈레톤 추가. `docker compose up api`로 실행, `POST /jobs`는 pending `job.json` 생성, `GET /jobs/{job_id}`는 상태 조회, `GET /jobs/{job_id}/video`는 렌더 완료 mp4 반환.
...
- [ ] **B2 — 백그라운드 실행 분리**. API 요청은 즉시 pending job을 반환하고, 실제 렌더는 별도 worker/thread/subprocess에서 돌리기. 아직 Celery/Redis 없이 단일 서버용 최소 구조로 시작.
...
- **API(B1)**: `docker compose up api` 후 `http://localhost:8000/docs`에서 확인. 현재 `POST /jobs`는 manifest만 만들고 렌더를 실행하지 않음(B2에서 분리 실행 추가 예정).
